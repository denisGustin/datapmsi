...
*Temps de lecture et mise en pratique : 1 heure à 2 heures*
...
Cette note est à consulter et pratiquer avant de participer à une de nos formations **Data PMSI avec R**. 
Nous vous invitons à copier/coller et exécuter les blocs de code proposés ci-dessous en s'assurant que vous les comprenez bien.  

Cette note est volontairement simplifiée au maximum, avec donc quelques abus de langage assumés[^1]

Si vous pratiquez déjà R, vous pouvez vous contenter d'un survol de la note, créer un projet dédié à la formation et charger les packages et le jeu de données [`mini_fake_dataset_mco`](dataset_mco.qmd).  
...
## 3eme étape : Créer un projet et un premier fichier .R 
...
## 4eme étape : Minimum à connaître  
...
## 5eme étapes : Installer et charger les packages  
...
## 6eme étape : Charger un jeu de données
...
## 7eme étape : Le pipe `%>%` et son fonctionnement  
...
## 8eme étape : Quelques manipulations simples  
...
-   on a chargé le jeu [`mini_fake_dataset_mco`](dataset_mco.qmd) et vérifié qu'on le voyait bien
...
[^5]: il y a aussi les données `Date` par exemple dont nous parlerons dans la formation.  
